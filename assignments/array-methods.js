...
//   fullName.push(name)
// })
runners.forEach(elements => fullName.push(`${elements.first_name} ${elements.last_name}`));
...
let allCaps = [];
allCaps = runners.map(elements => elements.first_name.toUpperCase());
console.log(allCaps); 
...
let largeShirts = [];
largeShirts= runners.filter(elements=> elements.shirt_size==='L');
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.reduce((starter, elements) => starter + elements.donation, 0);
